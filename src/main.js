...
function endPatchDrawing(){drawingPatch=false;draftPatch=[];$('#patchDrawing').hidden=true;$('#overlay').style.pointerEvents='none';syncPatchControls();}
$('#drawPatch').onclick=()=>{if(!ready)return;drawingPatch=true;draftPatch=[];if(matchMedia('(max-width:650px)').matches&&!$('.workspace').classList.contains('panel-hidden'))$('#sidebarToggle').click();$('#patchDrawHint').textContent='Click or tap corners around your visible sky, then Finish. Pan to your view before drawing.';$('#patchDrawing').hidden=false;$('#overlay').style.pointerEvents='auto';$('#finishPatch').disabled=true;$('#undoPatch').disabled=true;syncPatchControls();};
