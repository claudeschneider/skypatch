...
export function panSky(canvas,engine,dx,dy){
 const o=engine.stel.core.observer,scale=engine.fov/Math.min(canvas.clientWidth,canvas.clientHeight);engine.direction(o.yaw*180/Math.PI-dx*scale/Math.max(.2,Math.cos(o.pitch)),Math.max(-89.9,Math.min(89.9,o.pitch*180/Math.PI+dy*scale)));
}
export function installTouch(canvas,engine,{drawing=false}={}){
 ...
 canvas.addEventListener('touchmove',e=>{
  ...
  if(previous&&next.n===previous.n){
   ...
   if((drawing&&next.n>1)||(!drawing&&next.n===1))panSky(canvas,engine,next.x-previous.x,next.y-previous.y);
