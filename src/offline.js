...
export async function installOffline(){
 ...
 const button=document.createElement('button');button.className='offline-entry';button.textContent='↓ Install';document.querySelector('.header-actions').prepend(button);
 ...
 installMobilePrompt(button.onclick,button,dialog);
 const history=document.createElement('details');history.innerHTML='<summary>Version history / changelog</summary><pre class=release-history>Open to load release notes.</pre>';dialog.querySelector('.dialog-actions').before(history);history.ontoggle=async()=>{if(history.open)try{const response=await fetch(new URL('changelog.txt',base));if(!response.ok)throw Error();history.querySelector('pre').textContent=await response.text();}catch{history.querySelector('pre').textContent='Release notes unavailable. Update the app online to save them for offline use.';}};
 ...
 try{
  ...
  button.dataset.offlineReady='true';button.dispatchEvent(new Event('readinesschange'));
  await refresh();
