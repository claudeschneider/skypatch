...
## First session
1. Open [the planner](https://claudeschneider.github.io/skypatch/app/).
2. Set your observing location. The initial Vancouver coordinates are an example.
3. Choose a planning date and time; the displayed timezone belongs to your device.
4. Pan the sky or search for a target. Filters refine the map and list; search can find excluded targets too.
5. Select a target and choose Frame target. Edit the angular field dimensions if needed. Pan to compose within the centred frame.
...
- Desktop Chrome/Edge: use the Install button or browser's app-install control.
- Recent Safari on Mac: File → Add to Dock.

Installation is optional and browser support varies. Offline caching works on desktop as well as mobile after preparation; a browser bookmark can be used where installation is unavailable. If a newly installed app does not report ready, open it online and let its own core save before travelling.
...
The sky background follows the Sun’s altitude at your selected location and planning time: black at astronomical night, deep blue through twilight, and light blue in daylight. Stars and targets remain available for planning; this illustrative tint does not simulate whether they can actually be observed or photographed in daylight.

In **Info → When to observe**, view the altitude curve and start/end windows for the observing day (local noon to noon in the displayed device timezone). Green bands and My Sky windows show when the target centre is both above the horizon and inside your saved patch, even if its filter is switched off. The preview slider changes planning time. Times are approximate at one-minute sampling; very brief crossings may be missed. Daylight is included and shaded, and catalogue brightness/altitude filters do not alter the prediction.
