...
template=template.replace('__CACHE__',"'skypatch-core-'+new URL('./',self.location.href).pathname+'-"+digest.hexdigest()[:16]+"'").replace('__FILES__',json.dumps(files)).replace('__VERSION__',json.dumps(json.loads((root/'package.json').read_text())['version']+' ('+digest.hexdigest()[:12]+')'))
