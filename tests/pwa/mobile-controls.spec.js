...
test('Mobile time visibility persists and installation prompt dismisses without returning',async({page})=>{
 ...
 await expect(page.locator('#mobileInstall')).toBeVisible();await expect(page.locator('.install-nudge')).toBeVisible({timeout:10000});
 ...
 await page.locator('#mobileInstall').click();await expect(page.locator('.offline-dialog')).toBeVisible();await page.locator('#closeOffline').click();
...
test('Installed mobile app hides install promotion',async({page})=>{
 ...
 await expect(page.locator('#mobileInstall')).toBeHidden();await expect(page.locator('.install-nudge')).toBeHidden();
});
